...
const BloodPressurePatientMeal = ({route, navigation}) => {
  const {bloodPressurePatient} = route.params;
  ...
  const [
    gotbloodPressurePatientrenMealHistory,
    setGotbloodPressurePatientrenMealHistory,
  ] = useState(false);
  const [
    bloodPressurePatientrenMealHistory,
    setbloodPressurePatientrenMealHistory,
  ] = useState([]);
  ...
  useEffect(() => {
    ...
    Axios.post(backendUrl + 'getAllMealHistory', {
      ...
      .then(res => {
        if (isSubscribed) {
          setbloodPressurePatientrenMealHistory(res.data);
          setGotbloodPressurePatientrenMealHistory(true);
          handlebloodPressurePatientrensMealHistory();
        }
      })
      .catch(error => {
        // console.log(error);
    ...
    Axios.get(backendUrl + 'bloodPressurePatientFood')
      .then(res => {
        if (isSubscribed) {
  ...
  const handlebloodPressurePatientrensMealHistory = () => {
    ...
    if (bloodPressurePatientrenMealHistory.length > 0) {
      for (let i = 0; i < bloodPressurePatientrenMealHistory.length; i++) {
        if (
          bloodPressurePatientrenMealHistory[i].person_id ==
          bloodPressurePatient.id
        ) {
          let valueStart =
            bloodPressurePatientrenMealHistory[i].time_for_meal_taken +
            ':00:00';
          ...
          if (bloodPressurePatientrenMealHistory[i].date_for_meal != today) {
  ...
  const saveMeal = () => {
    ...
    Axios.post(backendUrl + 'savebloodPressurePatientMeal', {
      userEmail,
      meal: mealToSave,
      id: bloodPressurePatient.id,
    })
      .then(res => {
        navigation.navigate('bloodPressurePatientDetails', {
          userEmail,
  ...
  return (
    <SafeAreaView>
      {isLoadingFood || gotbloodPressurePatientrenMealHistory == false ? (
        <View
          style={{
            height: '100%',
            alignItems: 'center',
          ...
          {takeAnotherMeal ? (
            ...
            <View
              ...
              <Text style={{color: colors.brown, textAlign: 'center'}}>
                ...
                {bloodPressurePatient.names} after an hour.
              </Text>
            </View>
          )}
        </>
...
export default BloodPressurePatientMeal;
